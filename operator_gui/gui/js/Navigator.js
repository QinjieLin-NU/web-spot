 ...
 */
 NAV2D.Navigator = function(options) {
    ...
    var robotMarker = new ROS2D.ArrowShape({
      size : 10,
      strokeSize : 1,
      fillColor : createjs.Graphics.getRGB(0, 128, 0, 0.66),
      pulse : false
    });
    ...
    poseListener.subscribe(function(data) {
      // update the robots position on the map
      robotMarker.x = data.position.x;
      robotMarker.y = -data.position.y;
      if (!initScaleSet) {
      ...
      // change the angle
      robotMarker.rotation = stage.rosQuaternionToGlobalTheta(data.orientation);
