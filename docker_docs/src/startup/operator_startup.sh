...
sudo grep -F "source /root/spot_ws/devel/setup.bash" ~/.bashrc || sudo echo "source /root/spot_ws/devel/setup.bash" >> ~/.bashrc
source $HOME/.bashrc
...
cd  /root/spot_ws;\
roslaunch spot_config gazebo.launch & sleep 5; \
roslaunch spot_config slam.launch rviz:=true & sleep 2; \
