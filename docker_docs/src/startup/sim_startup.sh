...
#include ros source to bashrc
sudo grep -F "source /opt/ros/melodic/setup.bash" ~/.bashrc || sudo echo "source /opt/ros/melodic/setup.bash" >> ~/.bashrc
...
cd  /root/exoedu-robot/webot_worlds/web;\
roscore & sleep 2; \
python3 -m http.server >/root/web_output.txt & sleep 2; \
xvfb-run -a webots --no-sandbox  --stdout --stderr --mode=fast --no-rendering --stream --batch /usr/local/webots/projects/robots/boston_dynamics/spot/worlds/spot.wbt 
